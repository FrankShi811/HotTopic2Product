...
export default function App() {
  ...
  return <div className="app-shell" data-theme={theme}>
    ...
    <header className="topbar">
      <a className="brand" href="/" aria-label="Signal Forge 工作台首页"><span className="brand-mark" aria-hidden="true"><Layers3 size={18} strokeWidth={2.4} /></span><span>Signal Forge</span><small>OPERATIONS</small></a>
      <nav aria-label="主导航"><a className={view === 'studio' ? 'nav-link active' : 'nav-link'} href="/">工作台</a><a className={view === 'catalog' ? 'nav-link active' : 'nav-link'} href="/?view=catalog">概念库</a></nav>
      <div className="topbar-actions"><span className={`mode-chip ${health?.mode === 'gemini' ? 'live' : ''}`}><span aria-hidden="true" /> {health?.mode === 'gemini' ? '服务端模型' : '本地演示'}</span><button className="icon-button" type="button" aria-label={theme === 'night' ? '切换为浅色主题' : '切换为深色主题'} onClick={() => setTheme(theme === 'night' ? 'light' : 'night')}>{theme === 'night' ? <Sun size={17} /> : <Moon size={17} />}</button></div>
    </header>
    <main id="main-content" className="workspace" tabIndex={-1}>
      ...
      {!loading && dashboard && (view === 'studio' ? <StudioView dashboard={dashboard} health={health} selectedSignal={selectedSignal} selectedSignalId={selectedSignalId} busyId={busyId} refreshing={refreshing} onSelectSignal={updateSelectedSignal} onImport={importSignal} onCreateIdea={createIdea} onRefresh={() => void load(true)} /> : <CatalogView ideas={visibleIdeas} stageFilter={stageFilter} busyId={busyId} onReview={reviewIdea} onInterest={recordInterest} />)}
    </main>
  </div>;
}

function StudioView({ dashboard, health, selectedSignal, selectedSignalId, busyId, refreshing, onSelectSignal, onImport, onCreateIdea, onRefresh }: { dashboard: DashboardPayload; health: HealthPayload | null; selectedSignal: Signal | undefined; selectedSignalId: string; busyId: string | null; refreshing: boolean; onSelectSignal: (id: string) => void; onImport: (event: FormEvent<HTMLFormElement>) => void; onCreateIdea: (event: FormEvent<HTMLFormElement>) => void; onRefresh: () => void; }) {
  return <>
    <section className="overview" aria-labelledby="workspace-title"><div><p className="eyebrow">运营决策工作台</p><h1 id="workspace-title">从有证据的信号，走到可审核的产品假设。</h1><p className="lede">先记录来源，再生成概念，再由人批准。系统不会把示例数据或兴趣反馈误报成真实市场需求。</p></div><figure className="signal-figure"><img src="/assets/signal-to-object.png" width="1500" height="1000" alt="由织物、陶瓷和半透明配件组成的概念物件静物" fetchPriority="high" /><figcaption>概念插图，仅用于解释工作流</figcaption></figure></section>
